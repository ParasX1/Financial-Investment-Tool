import { GraphSettings } from './graphSettingsModal';
import { stockDataMap } from './StockCardComponent';

export interface MetricsResponse {
    metricType: 'OHLC';
    series: { ticker: string; ohlc: typeof stockDataMap[string] }[];
  }

interface FetchMetricsRequest {
  tickers: string[];
  settings: GraphSettings | null;
}

export async function fetchMetrics(
    req: FetchMetricsRequest
  ): Promise<MetricsResponse> {
    await new Promise((r) => setTimeout(r, 250));
  
    const series = req.tickers.map((ticker) => {
      const all = stockDataMap[ticker] ?? [];
      if (req.settings) {
        const { startDate, endDate } = req.settings.metricParams;
        const start = new Date(startDate);
        const end = new Date(endDate);
        const filtered = all.filter(d => {
          const dt = new Date(d.date);
          return dt >= start && dt <= end;
        });
        return { ticker, ohlc: filtered };
      }
      return { ticker, ohlc: all };
    });
  
    return { metricType: 'OHLC', series };
  }