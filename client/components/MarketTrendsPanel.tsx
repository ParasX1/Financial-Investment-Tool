...
function Sparkline({ data, base, w, h }: SparkProps) {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: 'block' }}>
      {yBase != null && (
        <line x1={0} x2={w} y1={yBase} y2={yBase}
          stroke="#555" strokeDasharray="2 4" strokeWidth="1" opacity={0.7} />
      )}
...
export function useQuoteAndLine(symbol: string) {
...
/* ---------- Overview tile (new UI style) ---------- */
interface OverviewTileProps {
  label: string;
  symbol: string;
  priceDigits?: number;
}
function OverviewTile({ label, symbol, priceDigits = 2 }: OverviewTileProps) {
  ...
  return (
    <Box sx={{
      ...
      <Typography sx={{ fontSize: 12, fontWeight: 700, color: '#5a8fff' }}>{label}</Typography>
      ...
      <Box sx={{ color }}>
        <Sparkline data={series} base={prev ?? undefined} w={100} h={28} />
      </Box>
      <Typography sx={{ fontSize: 12, fontWeight: 700, color }}>
        {change != null && pct != null
          ? `${up ? '+' : ''}${change.toFixed(2)} (${pct.toFixed(2)}%)`
          : '—'}
      </Typography>
    </Box>
  );
...
export default function MarketTrendsPanel({
  ...
  return (
    ...
    <Box sx={{
      ...
      <Typography sx={{ fontWeight: 700, fontSize: 16, color: 'white', mb: 1.5 }}>
        Today&apos;s Trend
      </Typography>
      <Grid container spacing={1.5}>
        {overviewItems.map((it) => (
          <Grid key={it.label} item xs={6} md={4}>
            <OverviewTile
              label={it.label}
              symbol={it.symbol}
...
//       <Grid container spacing={1.5}>
//         {overviewItems.map((it) => (
//           <Grid key={it.label} item xs={6} sm={4} md={2}>
//             <OverviewTile
//               label={it.label}
//               symbol={it.symbol}
//               priceDigits={(it as any).priceDigits}
...
// // import { GridView as GridViewIcon, ViewStream as ViewStreamIcon } from '@mui/icons-material';
...
// //         <Typography sx={{ color: 'white', fontSize: 18, fontWeight: 'bold' }}>Today's Trend</Typography>
// //         <Tooltip title={mode === 0 ? 'Switch to list view' : 'Switch to grid view'}>
// //           <IconButton size="small" onClick={() => setMode(mode === 0 ? 1 : 0)} sx={{ color: '#bbb', border: '1px solid #555' }}>
// //             {mode === 0 ? <ViewStreamIcon fontSize="small" /> : <GridViewIcon fontSize="small" />}
// //           </IconButton>
// //         </Tooltip>
// //       </Box>
