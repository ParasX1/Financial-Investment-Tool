...
import { FOCUS_VISIBLE, cn, feedbackToneClasses, fitType } from "../design";
...
export function CommunityNotice({ children }: { children: React.ReactNode }) {
  return (
    <div
      className={cn(
        "rounded-lg bg-[#5367ff]/10 px-4 py-3 text-[#dbe4ff]",
        fitType.bodySm,
...
export function StatusMessage({
  ...
  return (
    ...
    >
      <p className={fitType.control}>{title}</p>
      ...
      {items.map((item) => (
        ...
        >
          <div className="min-w-0">
            <p className={fitType.control}>{item.title}</p>
            {item.message ? (
              <p
                className={cn("mt-1 opacity-90", communityStyles.wrapAnywhere)}
              >
                {item.message}
              </p>
            ) : null}
