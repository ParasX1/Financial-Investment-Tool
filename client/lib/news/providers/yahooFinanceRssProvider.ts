...
function allowsBroadFallback(request: ServerNewsRequest) {
  if (request.userSearch || request.kind === "ticker") return false;

  return Boolean(buildStrictSearchText(request));
}

export const yahooFinanceRssProvider: NewsProvider = {
  id: "yahoo-finance-rss",
  label: "Yahoo Finance RSS",
  allowBroadFallback: allowsBroadFallback,
