...
class Serializer(t.Generic[_TSerialized]):
    ...
    def __init__(self: Serializer[str],
                 ...
                 serializer_kwargs: dict[str,
                                         t.Any] | None = None,
                 signer: type[Signer] | None = None,
                 signer_kwargs: dict[str,
                                     t.Any] | None = None,
                 fallback_signers: list[dict[str,
                                             t.Any] | tuple[type[Signer],
                                                            dict[str,
                                                                 t.Any]] | type[Signer]] | None = None,
                 ): ...

    # Serializer[bytes] with a bytes data serializer positional argument.
    @t.overload
    def __init__(self: Serializer[bytes],
                 secret_key: str | bytes | cabc.Iterable[str] | cabc.Iterable[bytes],
                 salt: str | bytes | None,
    ...
    def __init__(self,
        ...
        self.fallback_signers: list[dict[str,
                                         t.Any] | tuple[type[Signer],
                                                        dict[str,
                                                             t.Any]] | type[Signer]] = fallback_signers
        ...
        try:
            if is_text:
                return use_serializer.loads(
                    payload.decode("utf-8"))  # type: ignore[arg-type]
    ...
    def iter_unsigners(self, salt: str | bytes |
                       None = None) -> cabc.Iterator[Signer]:
    ...
    def dumps(self, obj: t.Any, salt: str |
              bytes | None = None) -> _TSerialized:
    ...
    def dump(self, obj: t.Any, f: t.IO[t.Any],
             salt: str | bytes | None = None) -> None:
