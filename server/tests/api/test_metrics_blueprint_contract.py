...
def test_metrics_route_serializes_series_before_building_metadata(client):
    drawdown = pd.Series(
        [-0.1, -0.2],
    ...
    with patch(
        "src.server.calculate_drawdown",
        return_value={"AAPL": drawdown},
    ):
...
def test_metrics_route_forwards_value_at_risk_confidence_level(client):
    with patch(
        "src.server.calculate_value_at_risk",
        return_value={"AAPL": 0.08},
    ) as calculator:
        response = client.post(
...
def test_metrics_route_forwards_efficient_frontier_options(client):
    with patch(
        "src.server.calculate_efficient_frontier",
        return_value={
            "asset_order": ["AAPL"],
            "returns": [0.12],
            "risks": [0.2],
